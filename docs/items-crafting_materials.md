# Crafting Materials

Resources used to create items and progress through crafting systems.

### Custom Powders (Crafting Materials)

These powders are distinct from consumable powders and are primarily used as crafting ingredients for various items.

-   **Amethyst Powder:** Used in crafting.
-   **Diamond Powder:** Used in crafting.
-   **Emerald Powder:** Used in crafting.
-   **Echo Dust:** Used in crafting.
-   **Quartz Powder:** Used in crafting.
-   **Slimeball Dust:** Used in crafting.
-   **Suspicious Substance:** Used in crafting.

### Sulfur Materials *(Added v1.8.0)*

Sulfur-based materials used in fire-themed crafting.

- **Sulfur Powder** — Base crafting material. Obtain from sulfur ore or mob drops.
- **Potent Sulfur Powder** — High-tier crafting component. Craft by combining 9× Sulfur Powder at a crafting table. Used as a premium ingredient in Inferno armor and fire weapon recipes.

**Uses:** Primarily as ingredients in crafting recipes for advanced items, equipment, and other custom mod content.

### Gems

Custom gems obtained by mining gem ores in the Overworld. Raw gems drop from ore blocks and can be smelted into polished gems.

**Raw Gems (from mining):**
- **Raw Citrine** — Smelts into Citrine. Common, found Y -16 to 64.
- **Raw Ruby** — Smelts into Ruby. Uncommon, found Y -64 to 32.
- **Raw Sapphire** — Smelts into Sapphire. Uncommon, found Y -64 to 32.
- **Raw Tanzanite** — Smelts into Tanzanite. Rare, found Y -64 to 16.
- **Raw Zircon** — Smelts into Zircon. Rare, found Y -64 to 16.
- **Raw Pink Garnet** — Smelts into Pink Garnet. Very rare, deepslate only below Y -16.

**Polished Gems (from smelting):**
- **Citrine, Ruby, Sapphire, Tanzanite, Zircon, Pink Garnet** — Used as crafting ingredients for advanced items and equipment.

**Uses:** Gems are used in crafting recipes for armor, weapons, and other custom mod content. Rarer gems are required for higher-tier crafting.

See [Custom Blocks - Gem Ores](./blocks.md#gem-ores) for full mining details and ore distribution.

### Quest Materials

Items required for quest completion or obtained as quest rewards.

**Common Materials:**
- Various vanilla Minecraft items
- Custom tokens and certificates
- Proof of completion items