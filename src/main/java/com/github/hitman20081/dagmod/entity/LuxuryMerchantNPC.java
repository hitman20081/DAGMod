...
public class LuxuryMerchantNPC extends PathfinderMob implements Merchant {
    ...
    private void sendUnlockHints(ServerPlayer player, boolean advanced, boolean legendary) {
        if (!advanced) {
            player.sendSystemMessage(Component.literal("[Trophy Dealer] ").withStyle(ChatFormatting.DARK_PURPLE, ChatFormatting.BOLD)
                    .append(Component.literal("You look green. Stand watch through a few nights and I'll show you my real stock.").withStyle(ChatFormatting.GRAY)));
        } else if (!legendary) {
            player.sendSystemMessage(Component.literal("[Trophy Dealer] ").withStyle(ChatFormatting.DARK_PURPLE, ChatFormatting.BOLD)
                    .append(Component.literal("Dragon trophies require a dragonslayer. Prove it and I'll open the vaults.").withStyle(ChatFormatting.DARK_RED)));
        }
    }
    ...
    @Override
    public SoundEvent getNotifyTradeSound() { return SoundEvents.UI_TOAST_CHALLENGE_COMPLETE; }
